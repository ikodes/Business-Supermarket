...
{    
  ...
  public function actionLogin()
  {
        ...
        {     
          $model->attributes=$_POST['LoginForm'];
                ...
                if($model->validate() && $model->login()){  
                   ...
                   $log = new Businesslogtransaction(); 
    ...
    if(isset($_POST['user_default_business_lost_email']))
  {
              $codeActive =  Yii::app()->getRequest()->getParam('user_default_business_lost_email');
                
             if(isset($codeActive)){
                      
               $userData =  User::model()->findAllByAttributes(array('user_default_business_email'=>$codeActive));
               
                   if(!empty($userData)){ 
                           $newpass = rand(0,9999999999);
                           $template =  Mailtemplate::model()->findByAttributes(array('template_id'=>3)); 
                           $postRecord = User::model()->findByPk($userData[0]['user_default_business_id']);
                           $postRecord->user_default_business_active_link='';
                           $postRecord->user_default_business_pstatus='1';
                           $postRecord->user_default_business_pass=SharedFunctions::app()->encrypt_code($newpass); ;
                           $postRecord->save(); 
                           
                           $string = array(
                                    '{{#USERNAME#}}'=>ucwords($userData[0]['user_default_business_name'] .' '.$userData[0]['user_default_business_surname']),
                                    ...
                                    /*
                                    '{{#USEREMAIL#}}'=>ucwords($userData[0]['user_default_business_name'] .' '.$userData[0]['user_default_business_surname']),
                            ...
                            SharedFunctions::app()->sendmail($userData[0]['user_default_business_email'],$template->template_subject,$body); 
                               $msg = array('success'=>'true','redirect'=>Yii::app()->user->returnUrl); 
