...
class UserMessages extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_default_business_blid, subject, message, user_default_business_id, likes_total, dislikes_total, attachement, created_date', 'required'),
			array('user_default_business_blid, user_default_business_id, likes_total, dislikes_total', 'numerical', 'integerOnly'=>true),
			array('subject', 'length', 'max'=>250),
			array('attachement', 'length', 'max'=>255),
			array('is_spam, first_message', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_default_business_blid, subject, message, user_default_business_id, likes_total, dislikes_total, attachement, is_spam, first_message, created_date', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'user_default_business_blid' => 'Blist',
			'subject' => 'Subject',
			'message' => 'Message',
			'user_default_business_id' => 'User',
    ...
    public function getPostComments($comment_id){
        ...
        //$sql .= " ON c.user_default_business_id = u.drg_id ";
    ...
    public static function getUserStats($listingId){
        ...
        $sql = "SELECT u.drg_id as user_default_business_id, u.drg_username as username,u.drg_image,u.drg_name,u.drg_surname, sum(likes_total) as like_total, sum(dislikes_total) as dislike_total, (sum(likes_total) - sum(dislikes_total)) as user_reputation ";
        $sql .= "FROM drg_user_messages c ";
        $sql .= "INNER JOIN drg_user u ";
        $sql .= "ON c.user_default_business_id = u.drg_id ";
        $sql .= "WHERE c.user_default_business_blid =".$listingId ;
        $sql .= " GROUP BY c.user_default_business_id";
        ...
        if( $usersStats ){
            ...
            foreach ($usersStats as $userStats) {
                ...
                $result[$userStats['user_default_business_id']] = $userStats;
