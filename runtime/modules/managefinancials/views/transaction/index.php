      ...
      <div id="registration-tabs"> <a href="javascript:void(0);">My Account</a>
        ...
        <div id="content" style="margin-top:-17px;">
           ...
           $currencymodel = new Currency();
            $currenyCode=Currency::model()->getCurrencyCode($model->user_default_currency);
       ...
       //     $lastTransaction = (UserFinancialTransaction::model()->lastTransactionCode()!="") ? UserFinancialTransaction::model()->lastTransactionCode(): $currenyCode;
         ...
         //  echo $symbol; exit;
			if($model->user_default_currency == 1){
				$currenySign = '($) US Dollar';
			}
		else if($model->user_default_currency == 2){
				$currenySign = '(£) British Pound';
			}else if($model->user_default_currency == 3){
				$currenySign = '(€) Euro';
			}
            ...
            <div id="balance">
                    ...
                    <input type="text" value="<?php echo $x->convert(number_format($accountBalance[0]['user_default_account_balance_account_balance'],2),$accountBalance[0]['user_default_account_balance_currency_code'],$currenyCode); ?>" readonly=readonly name="addFundsw" id="accountBalanceValue" />
            ...
            <table class="gernal_table ManageFinancials" width="50%" cellspacing="2" cellpadding="1" bordercolor="#fff" border="0">
                <tbody>
					...
					foreach($dataProvider AS $index=>$record):
						...
						?>
                        <tr class="<?php echo $class;?>" <?php if($record['user_default_financial_transaction_withdraw_status'] =='1'){?> style="color:red;" <?php } ?>>
                        <td><?php echo $record['user_default_transaction_date'];?></td>
                        <td><?php echo $record['user_default_transaction_type'];?></td>
                        <td><?php echo $record['user_default_transaction_details'];?></td>
                        <td><?php echo $record['user_default_transaction_paypal_transactionId'];?></td>
                        <td><?php echo ($record['user_default_transaction_paid_out'] !='0.00')?$x->convert($record['user_default_transaction_paid_out'], $record['user_default_financial_transaction_currency_code'],$currenyCode):'';?></td>
                        <td><?php echo ($record['user_default_transaction_paid_in'] !='0.00')?$x->convert($record['user_default_transaction_paid_in'],$record['user_default_financial_transaction_currency_code'],$currenyCode):'';?></td>
						<td><?php echo $x->convert($record['user_default_transaction_balance'],$record['user_default_financial_transaction_currency_code'],$currenyCode);?></td>
                    </tr>
				<?php 	 endforeach;?>
