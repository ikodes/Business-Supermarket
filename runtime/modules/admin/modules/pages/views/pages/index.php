...
<div class="user_listing_search"><br> 
    ...
    <div>
	 <h2 align="center" class="Blue" style="margin:15px 0;">Create / Update Site Pages  </h2> 
<div style="height: 39px;"><a href="<?php echo $this->createUrl('/admin/pages/pages/create');?>" class="button blue" style="float:right">+Add New Page</a></div>
	  <div>
...
<td class="title_column" style="cursor:default" title="Listing title">Page Title</td>
<td class="title_column" style="cursor:default" title="Listing title">Controller Name</td>
          ...
          if($list > 0){        foreach($list as $row){ 
                    if($i%2==0){
                    ?>
                        <tr onmouseover="ChangeColorGrey(this, true);" onmouseout="ChangeColorGrey(this, false);"  onclick="DoNav('<?php echo Yii::app()->createUrl('admin/pages/pages/create/id/'.$row->id);?>');" class="GreyRow">
                    <?php    
                    }else {
                    ?>
                         <tr onmouseover="ChangeColorMauve(this, true);" onmouseout="ChangeColorMauve(this, false);" onclick="DoNav('<?php echo Yii::app()->createUrl('admin/pages/pages/create/id/'.$row->id);?>');" class="MauveRow">
         
                    <?php     
                    }
                    ?>
                        <td><?php  echo $row->pname;?></td>
                             <td><?php  $slug=$row->pslug; if($slug!="") { $cslug=explode('/',$slug); } echo $cslug[0]; ?></td>              
                        <td><?php  echo $cslug[1];?></td></tr>    
