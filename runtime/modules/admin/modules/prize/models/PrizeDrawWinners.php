 ...
 * @property integer $id
 * @property string $user_default_id
...
class PrizeDrawWinners extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_default_id, date_draw, prize_won_amount, payment_ref', 'required'),
			array('user_default_id', 'length', 'max'=>25),
			array('prize_won_amount', 'length', 'max'=>10),
			array('payment_ref', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_default_id, date_draw, prize_won_amount, amount_paid_date, payment_ref', 'safe', 'on'=>'search'),
		);
	}
