...
<input size="4" maxlength="4" id="year" style="width:50px" onfocus="getNormal('#slider_title');" name="year" type="text" value="<?php echo $modelnew->year; ?>"> 
                   
<label for="Prize_month" class="required">Month : </label>
<input size="2" maxlength="2" id="year" style="width:50px" onfocus="getNormal('#slider_title');" name="month" type="text" value="<?php echo $modelnew->month; ?>">   
                 
<label for="Prize_prize_value">Prize Value ($) : </label> 
<input size="10" maxlength="10" name="prize_values" id="Prize_prize_value" type="text" value="<?php echo $modelnew->prize_value; ?>">                    

<label for="Prize_points_required">Points Required : </label> 
<input name="points_required" id="Prize_points_required" type="text" value="<?php echo $modelnew->points_required; ?>">    

<input type="hidden"  value="<?php echo $modelnew->id; ?>" name="prizeid">
...
$listData =  CHtml::listData(User::model()->findAll(array("order"=>'user_default_username asc')),'user_default_id','user_default_username');
                 echo CHtml::dropDownList('win_username','',$listData,array('class'=>'chzn-select','data-placeholder'=>'Please select','id'=>'win_username','style'=>'width:180px'));
...
<td><?php  $userid=$row->user_default_id;
$connection = Yii::app()->db;
...
echo $sliderfile = $sliderresult['user_default_username'];
...
<td><?php  echo $row->payment_ref;?></td>   
            ...
            ?>
        </table> <div class="clear"></div>		 <div id="page_nav" class="art-page-nav">                          <table class="sl-select" id="example">                            <tr>                              <td style="text-align: right; cursor: default;" title="Select number of records to view from the dropdown menu">View</td>                              <td>                              <?php                               if(isset($_REQUEST['rows'])){                            	$count = $_REQUEST['rows'];                            }else {                            	$count = 20;                            }                                                           ?>                              <select name="user_default_category" data-placeholder="12" class="chzn-select" style="width:60px;" tabindex="2" onchange="window.location.href = '?rows='+jQuery(this).val();">                                   <option <?php echo ($count==20) ? 'selected=selected':'';?> value="20">20</option>                                  <option <?php echo ($count==50) ? 'selected=selected':'';?> value="50">50</option>                                  <option <?php echo ($count==100) ? 'selected=selected':'';?> value="100">100</option>                                </select>                                                              </td>                            </tr>                          </table>                </div>                    <!-- Bottom navigation menu -->                       <?php $this->widget('CLinkPager',array('pages' => $pages,'header' => '',                        	'firstPageLabel' => '<',                        	'prevPageLabel' => 'previous',                        	'nextPageLabel' => 'next',                            'maxButtonCount'=>5,                        	'lastPageLabel' => '>','htmlOptions'=>array('name'=>'test','id'=>'navlist','class'=>'pager'))                         ); ?>                                           <!-- /Bottom navigation menu -->             <ul name="test" id="navlist" class="pager"></ul>            <div class="clear"></div>
