        ...
        <table class="gernal_table manageListing" border="0" bordercolor="#fff" style="" width="50%" cellpadding="1" cellspacing="2">
          ...
          if(!empty($list1)){
                ...
                foreach($list1 as $listData1){
                   if($j%2==0){
                   ?>
                     <tr onclick="DoNav('<?php echo Yii::app()->createUrl('businesslisting/create/blistid/'.$listData1->user_default_business_blid);?>');" onmouseover="ChangeColorGrey(this, true);" onmouseout="ChangeColorGrey(this, false);" class="GreyRow">
                   <?php           
                   }else {
                   ?>
                     <tr onclick="DoNav('<?php echo Yii::app()->createUrl('businesslisting/create/blistid/'.$listData1->user_default_business_blid);?>');" onmouseover="ChangeColorMauve(this, true);" onmouseout="ChangeColorMauve(this, false);"  class="GreyRow" style="background-color: rgb(234, 221, 237);">
                   <?php 
                   }
                   ?>
                    <td><?php if($listData1->user_default_business_blistingstatus == 1){
                            echo date('d-m-Y',strtotime($listData1->user_default_business_datetime));
                        }else{
                            echo 'Pending';
                        }?></td>
                    <td><?php echo $listData1->user_default_business_title;?></td>
                    <td><?php
                            $from=date_create(date('Y-m-d'));
                            $to=date_create(date('Y-m-d', strtotime($listData1->user_default_business_datetime)));
                    ...
                    <?php
                       ...
                       $visitpage_to=date_create(date('Y-m-d', strtotime($listData1->user_default_business_last_page_visit)));
                    $visitpage_diff=date_diff($visitpage_to,$visitpage_from);
                        echo $visitpage_diff->format('%R%a days');
                    ?>                    
                    </td>
                    <td><?php echo $listData1->user_default_business_page_visit; ?></td>
                    <td>0</td>
                    </tr>
                   <?php
                ...
                <div id="page_nav" class="art-page-nav">
                          <table class="sl-select" id="example">
                            <tr>
                              ...
                              ?>
                              <select name="user_default_business_category" data-placeholder="12" class="chzn-select" style="width:60px;" tabindex="2" onchange="window.location = '?rows='+$(this).val();">
          ...
          <?php 
             if(!empty($list)){
                ...
                foreach($list as $listData){
                    if($i%2==0){
                    ?>
                        <tr onmouseover="ChangeColorGrey(this, true);" onmouseout="ChangeColorGrey(this, false);"  onclick="DoNav('<?php echo Yii::app()->createUrl('businesslisting/create/blistid/'.$listData->user_default_business_blid);?>');" class="GreyRow">
                    <?php    
                    }else {
                    ?>
                         <tr onmouseover="ChangeColorMauve(this, true);" onmouseout="ChangeColorMauve(this, false);" onclick="DoNav('<?php echo Yii::app()->createUrl('businesslisting/create/blistid/'.$listData->user_default_business_blid);?>');" class="MauveRow">
         
                    <?php     
                    }
                    ?>
                        <td><?php  echo  date('d-m-Y',strtotime($listData->user_default_business_datetime));?></td>
                        <td><?php  echo $listData->user_default_business_title;?></td>                        
                        <td><?php  echo  $listData->user_default_business_status;?></td>
                      </tr>    
                    ...
                    <div id="page_nav" class="art-page-nav">
                      <table class="sl-select" id="example">
                        <tr>
                          ...
                          ?>
                          <select name="user_default_business_category" data-placeholder="12" class="chzn-select" style="width:60px;" tabindex="2" onchange="window.location = '?row='+$(this).val();">
                              <option <?php echo ($count==5) ? 'selected=selected':'';?> value="5">5</option>
                              <option <?php echo ($count==10) ? 'selected=selected':'';?> value="10">10</option>
                              <option <?php echo ($count==20) ? 'selected=selected':'';?> value="20">20</option>
                              <option <?php echo ($count==50) ? 'selected=selected':'';?> value="50">50</option>
