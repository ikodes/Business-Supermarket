...
class BusinesslistingController extends Controller
{
	...
	public function actionView($id)
	{
        ...
        $criteria1->select='t.*,tuser.user_default_business_name,tuser.user_default_business_slogon,tuser.user_default_business_image';
        $criteria1->join = 'LEFT OUTER JOIN `user_default_business` tuser on tuser.user_default_business_id = t.user_default_business_id';
        $model = Businesslisting::model()->find($criteria1);
        $this->pageTitle=$model->user_default_business_title. ' - Business Supermarket';        
        $this->pageTitle='Business Listing - Business Supermarket'; 
		$this->metaDesc=$model->user_default_business_whoweare;
        $this->metaKeys=$model->user_default_business_keyword;
	...
	public function actionBusiness_services(){
	    ...
	    if(isset($_REQUEST['date_sort']) && $_REQUEST['date_sort']=='oldest'){
	        ...
	        $sort_string = 'user_default_business_bapprovedate desc';
	    }
	    // $criteria1->order = 'user_default_business_lid desc';

	    if(isset($_REQUEST['name_sort'])){
	        if($_REQUEST['name_sort']=='z_a'){
	            $sort_string= 'tuser.user_default_business_first_name desc';
	        }else {
	            $sort_string= 'tuser.user_default_business_first_name asc';
	        }
	    }

	    if(isset($_REQUEST['origin_sort'])){
	        if($_REQUEST['name_sort']=='z_a'){
	            $sort_string= 'tcountry.user_default_country_name desc';
	        }else {
	            $sort_string= 'tcountry.user_default_country_name asc';
	        }
	    }
	    ...
	    if(!Yii::app()->user->isGuest){
	        ...
	        $criteria1->join.=' LEFT OUTER JOIN `user_default_business_favourite_blisting` fl ON fl.blisting_id=t.user_default_business_blid';
	...
	public function actionAdd_favourite()
    {
       ...
       $user_default_business_id = Yii::app()->user->id;
        ...
        if($fav_exists){
            ...
            $new_fav->user_default_business_id = $user_default_business_id;
	...
	public function actionCreate()
	{
		...
		$this->pageTitle='Create Business Listing - Business Supermarket'; 
        ...
        $model->user_default_business_last_page_visit = date('Y-m-d');
        $model->user_default_business_page_visit = $model->user_default_business_page_visit + 1;
        $model->save();
				...
				{	
				    if($_POST['btnsaveforlater']==1){ 
                       $this->redirect(Yii::app()->createUrl('business/myaccount/update'));     
                        ...
                        $this->redirect(Yii::app()->createUrl('businesslisting/business_listing_step2/blistid/'.$model->user_default_business_blid));
					}
	...
	public function actionbusiness_listing_step2(){
       ...
       if($_POST['user_default_business_imgdesc']){
            ...
            if(isset($_POST['old_img_name'])){
                ...
                for($i=0;$i<count($_POST['old_img_name']);$i++){ 
                    ...
                    if($_POST['old_img_name'][$i]!=""){
                        ...
                        $whosloggedin = Businesslistingimages::model()->find('user_default_business_listing_image=:imagename',array(':imagename'=>$_POST['old_img_name'][$i]) ); 
                        $whosloggedin->delete();
            ...
            for($i=0;$i<count($_POST['img_name']);$i++){ 
                 ...
                 if($_POST['img_name'][$i]!=""){
                    $businesslistingimages = new Businesslistingimages;
                         ...
                         $businesslistingimages->user_default_business_blid =  $blistid;
          ...
          /******************************/
            if(isset($_POST['user_default_business_old_videos'])){
                $i=0;
                for($i=0;$i<count($_POST['user_default_business_old_videos']);$i++){ 
                    
                    if($_POST['user_default_business_old_videos'][$i] !="")
                    { 
                        $whosloggedin = Businesslistingvideos::model()->find('user_default_business_listing_video=:videoname',array(':videoname'=>$_POST['user_default_business_old_videos'][$i]) ); 
                        $whosloggedin->delete();
            ...
            $i=0;            
            for($i=0;$i<count($_POST['user_default_business_videos']);$i++){ 
                    
                 if($_POST['user_default_business_videos'][$i] !=""){     	             
                     
                       $Businessvideo = new Businesslistingvideos; 
                     $video =  explode("/",$_POST['user_default_business_videos'][$i]);
                     $video = array_reverse($video);    
                     $videoData = ""; 
                     $Businessvideo->user_default_business_videodesc = $_POST['user_default_business_videodesc'][$i];
                     $Businessvideo->user_default_business_listing_video = $video[0];
					 $Businessvideo->user_default_business_listing_video_type = "0";
                     $Businessvideo->user_default_business_blid =  $blistid;
                     $Businessvideo->save();  
                 }   
            }
		  ...
		  /*
			...
			$i=0;            
            for($i=0;$i<=count($_POST['user_default_business_imgdesc']);$i++){ 
                ...
                for($i=0;$i<count($_POST['old_img_name']);$i++){ 
                 ...
                 if($_POST['img_name'][$i]!=""){
                      ...
                      if(isset($_POST['img_name'][$i])){
                        $imageData =  Businesslistingimages::model()->find("user_default_business_listing_image='".$_POST['img_name'][$i]."'");
                     }
                     if(!$imageData){
                         $businesslistingimages = new Businesslistingimages;
                         $businesslistingimages->user_default_business_listing_image = $_POST['img_name'][$i];
           ...
           {
               $model_user = Business::model()->find("user_default_business_id='".Yii::app()->user->getState('uid')."'");


               $to = $model_user['user_default_business_email'];		   
			   ...
			   $ltitle="<i>".$model['user_default_business_title']."</i>";
	   		        $ldate="<i>".$model['user_default_business_datetime']."</i>";
			        $lstatus="<i>".$stat."</i>";
    $string = array(
                        '{{#LISTINGTITLE#}}'=>ucwords($ltitle),
                       '{{#USERNAME#}}'=>ucwords($model_user['user_default_business_first_name'].' '.$model_user['user_default_business_surname']),
...
$stat ="Permenant Deleted";
 ...
 $yii_user_request_id = '<a href="'.Yii::app()->getBaseUrl(true)."/"."businesslisting/fupdate/blistid/".$model->user_default_business_blid.'" target="_blank" >here >> </a>';
    ...
    $string = array(
					...
					$subject="Listing ".$model['user_default_business_title']." is waiting publication";
	...
	public function actionbusiness_listing_step3()
	{
		...
		if(isset($_POST['Businesslisting'])){
            ...
            $model->user_default_business_testimonial = $_POST['Businesslisting']['user_default_business_testimonial'];
			$model->user_default_business_status = 0;
            $model->user_default_business_keyword = $_POST['Businesslisting']['user_default_business_keyword'];
            if($model->save()){   
					...
					if($_POST['saveforlater'])
					{
						 $model_user = Business::model()->find("user_default_business_id='".Yii::app()->user->getState('uid')."'");
						   ...
						   $message = "
						   <table>
						   	<tr><p>Dear ".$model_user['user_default_business_first_name']."</p></tr>
							<tr><p>You have successfully submitted your listing.</p></tr>
							<tr><p>Company slogon:&nbsp;".$model['user_default_business_slogon']."<br/>
								Date of submission and Time:&nbsp;".$model['user_default_business_datetime']."<br/>
								Status:&nbsp;".$model['user_default_business_status']."</p></tr>
	...
	public function actionBusinesslistingimage(){  
	 ...
	 public function actionPreview_business_listing(){
		...
		$this->pageTitle='Preview Business Listing - Business Supermarket'; 
        
        $model = Businesslisting::model()->find("user_default_business_id = '".Yii::app()->user->getState('uid')."' and  user_default_business_blid ='".$id."'");
        $this->pageTitle=$model->user_default_business_title.' - Business Supermarket';  $userData = Business::model()->find("user_default_business_id = '".Yii::app()->user->getState('uid')."'");
       ...
       $path = $_SERVER['DOCUMENT_ROOT'].'/';  
        ...
        /*	if ($filesize > $max_upload_size) 
            ...
            */
            ...
            $destination_file_path=$user_video_path.$name.$flvext;
	...
	public function actionIndex()
	{
     ...
     //   $criteria1->compare('approved',1,true);
        $criteria1->compare('user_default_business_id',Yii::app()->user->getState('uid'),true); 
        $criteria1->order = 'user_default_business_blid desc';
        $total1 = Businesslisting::model()->count($criteria1); 
        ...
        **/
        ...
        $criteria->compare('user_default_business_id',Yii::app()->user->getState('uid'),true); 
    ...
    */ 
    ...
    public function actionSelectlisting(){ 
        ...
        $userData = Business::model()->find("user_default_business_id = '".Yii::app()->user->getState('uid')."'");   
